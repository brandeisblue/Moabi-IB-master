...
public class DailyReviewItem extends AbstractItem<DailyReviewItem, DailyReviewItem.ViewHolder> {

    private static final String TAG = DailyReviewItem.class.getSimpleName();
    private Fragment mFragment;
    private String mDate;

    public DailyReviewItem(Fragment fragment, String date) {
        mFragment = fragment;
        mDate = date;
    }
    ...
    @NonNull
    @Override
    public DailyReviewItem.ViewHolder getViewHolder(View v) {
        return new DailyReviewItem.ViewHolder(v);
    }

    @Override
    public int getType() {
        return R.id.daily_review_item;
    }
    ...
    protected static class ViewHolder extends FastAdapter.ViewHolder<DailyReviewItem> {
        ...
        private DailyReviewViewModel itemViewModel;
        ...
        @Override
        public void bindView(final DailyReviewItem item, List<Object> payloads) {
            Log.i(TAG, "DailyReviewItem is created");
            itemSharedPreferences = itemView.getContext().getSharedPreferences(
                    itemView.getContext().getString(R.string.com_ivorybridge_moabi_DAILY_REVIEW_SHARED_PREFERENCE_KEY),
        ...
        private void configureData(DailyReviewItem item, int numOfDay) {
            if (numOfDay == 1) {
        ...
        @Override
        public void unbindView(DailyReviewItem item) {
            //firebaseManager.getDailyReviewLevelTodayRef().removeEventListener(valueEventListener);
        }

        private void configureChartForOneDay(DailyReviewItem item) {
            itemViewModel.getEntries(formattedTime.getStartOfDay(item.mDate),
                    formattedTime.getEndOfDay(item.mDate)).observe(item.mFragment,
                    new Observer<List<DailyReview>>() {
                @Override
                public void onChanged(List<DailyReview> itemList) {
                    if (itemList != null) {
                        if (itemList.size() == 0) {
                            lineChart.isEmpty();
                            lineChart.clear();
        ...
        private void configureChartForExtendedTimeRange(DailyReviewItem item, int numOfDays) {
            itemViewModel.getDailyDailyReviews(formattedTime.getStartOfDayBeforeSpecifiedNumberOfDays(formattedTime.getCurrentDateAsYYYYMMDD(), numOfDays - 1),
                    formattedTime.getEndOfDay(formattedTime.getCurrentDateAsYYYYMMDD())).observe(item.mFragment, new Observer<List<DailyDailyReview>>() {
                @Override
        ...
        private void drawLineChart(List<Entry> lineEntries, List<String> entryDatesList, int numOfDays, float average) {
            ...
            NumeralChartMarkerView numeralChartMarker = new NumeralChartMarkerView(
                    ...
                    itemView.getContext().getString(R.string.daily_review_camel_case));

...
public class StressItem extends AbstractItem<StressItem, StressItem.ViewHolder> {

    private static final String TAG = StressItem.class.getSimpleName();
    private Fragment mFragment;
    private String mDate;

    public StressItem(Fragment fragment, String date) {
        mFragment = fragment;
        mDate = date;
    }
    ...
    @NonNull
    @Override
    public StressItem.ViewHolder getViewHolder(View v) {
        return new StressItem.ViewHolder(v);
    }

    @Override
    public int getType() {
        return R.id.stress_item;
    }
    ...
    protected static class ViewHolder extends FastAdapter.ViewHolder<StressItem> {
        ...
        private StressViewModel itemViewModel;
        ...
        @Override
        public void bindView(final StressItem item, List<Object> payloads) {
            Log.i(TAG, "StressItem is created");
        ...
        private void configureData(StressItem item, int numOfDay) {
            if (numOfDay == 1) {
        ...
        @Override
        public void unbindView(StressItem item) {
            //firebaseManager.getStressLevelTodayRef().removeEventListener(valueEventListener);
        }

        private void configureChartForOneDay(StressItem item) {
            itemViewModel.getEntries(formattedTime.getStartOfDay(item.mDate), formattedTime.getEndOfDay(item.mDate)).observe(item.mFragment, new Observer<List<Stress>>() {
                @Override
                public void onChanged(List<Stress> itemList) {
                    if (itemList != null) {
                        if (itemList.size() == 0) {
                            lineChart.isEmpty();
                            lineChart.clear();
        ...
        private void configureChartForExtendedTimeRange(StressItem item, int numOfDays) {
            itemViewModel.getDailyStress(formattedTime.getStartOfDayBeforeSpecifiedNumberOfDays(formattedTime.getCurrentDateAsYYYYMMDD(), numOfDays - 1),
                    formattedTime.getEndOfDay(formattedTime.getCurrentDateAsYYYYMMDD())).observe(item.mFragment, new Observer<List<DailyStress>>() {
                @Override
        ...
        private void drawLineChart(List<Entry> lineEntries, List<String> entryDatesList, int numOfDays, float average) {
            ...
            NumeralChartMarkerView numeralChartMarker = new NumeralChartMarkerView(
                    ...
                    itemView.getContext().getString(R.string.stress_camel_case));

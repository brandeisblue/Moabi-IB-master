...
public class Phq9Item extends AbstractItem<Phq9Item, Phq9Item.ViewHolder> {

    private static final String TAG = Phq9Item.class.getSimpleName();
    private Fragment mFragment;
    private String mDate;

    public Phq9Item(Fragment fragment, String date) {
        mFragment = fragment;
        mDate = date;
    }
    ...
    @NonNull
    @Override
    public Phq9Item.ViewHolder getViewHolder(View v) {
        return new Phq9Item.ViewHolder(v);
    }

    @Override
    public int getType() {
        return R.id.depression_item;
    }
    ...
    protected static class ViewHolder extends FastAdapter.ViewHolder<Phq9Item> {
        ...
        private DepressionViewModel itemViewModel;
        ...
        @Override
        public void bindView(final Phq9Item item, List<Object> payloads) {
            Log.i(TAG, "Phq9Item is created");
            itemSharedPreferences = itemView.getContext().getSharedPreferences(
                    itemView.getContext().getString(R.string.com_ivorybridge_moabi_PHQ9_SHARED_PREFERENCE_KEY)
        ...
        private void configureData(Phq9Item item, int numOfDay) {
            if (numOfDay == 1) {
        ...
        @Override
        public void unbindView(Phq9Item item) {
            //firebaseManager.getPhq9LevelTodayRef().removeEventListener(valueEventListener);
        }

        private void configureChartForOneDay(Phq9Item item) {
            itemViewModel.getEntries(formattedTime.getStartOfDay(item.mDate), formattedTime.getEndOfDay(item.mDate)).observe(item.mFragment, new Observer<List<Phq9>>() {
                @Override
                public void onChanged(List<Phq9> itemList) {
                    if (itemList != null) {
                        if (itemList.size() == 0) {
                            lineChart.isEmpty();
                            lineChart.clear();
        ...
        private void configureChartForExtendedTimeRange(Phq9Item item, int numOfDays) {
            itemViewModel.getDailyPhq9s(formattedTime.getStartOfDayBeforeSpecifiedNumberOfDays(formattedTime.getCurrentDateAsYYYYMMDD(), numOfDays - 1),
                    formattedTime.getEndOfDay(formattedTime.getCurrentDateAsYYYYMMDD())).observe(item.mFragment, new Observer<List<DailyPhq9>>() {
                @Override
        ...
        private void drawLineChart(List<Entry> lineEntries, List<String> entryDatesList, int numOfDays, float average) {
            ...
            NumeralChartMarkerView numeralChartMarker = new NumeralChartMarkerView(
                    ...
                    itemView.getContext().getString(R.string.depression_phq9_camel_case));

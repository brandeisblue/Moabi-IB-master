...
public class EnergyItem extends AbstractItem<EnergyItem, EnergyItem.ViewHolder> {

    private static final String TAG = EnergyItem.class.getSimpleName();
    private Fragment mFragment;
    private String mDate;

    public EnergyItem(Fragment fragment, String date) {
        mFragment = fragment;
        mDate = date;
    }
    ...
    @NonNull
    @Override
    public EnergyItem.ViewHolder getViewHolder(View v) {
        return new EnergyItem.ViewHolder(v);
    }

    @Override
    public int getType() {
        return R.id.energy_item;
    }
    ...
    protected static class ViewHolder extends FastAdapter.ViewHolder<EnergyItem> {
        ...
        private SharedPreferences moodAndEnergySharedPreferences;
        private SharedPreferences.Editor moodAndEnergySPEditor;
        ...
        private void configureData(EnergyItem item, int numOfDay) {
            if (numOfDay == 1) {
        ...
        @Override
        public void unbindView(EnergyItem item) {
            //firebaseManager.getMoodAndEnergyLevelTodayRef().removeEventListener(valueEventListener);
        }

        private void configureChartForOneDay(EnergyItem item) {
            moodAndEnergyViewModel.getEntries(formattedTime.getStartOfDay(item.mDate), formattedTime.getEndOfDay(item.mDate)).observe(item.mFragment, new Observer<List<MoodAndEnergy>>() {
                @Override
                public void onChanged(List<MoodAndEnergy> moodAndEnergyList) {
                            ...
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    for (MoodAndEnergy moodAndEnergy : moodAndEnergyList) {
                                        ...
                                        energyLineEntries.add(new Entry(timeOfDayInMinutes, energy));
                                    }
        ...
        private void configureChartForExtendedTimeRange(EnergyItem item, int numOfDays) {
            moodAndEnergyViewModel.getDailyEnergies(formattedTime.getStartOfDayBeforeSpecifiedNumberOfDays(formattedTime.getCurrentDateAsYYYYMMDD(), numOfDays - 1),
                    formattedTime.getEndOfDay(formattedTime.getCurrentDateAsYYYYMMDD())).observe(item.mFragment, new Observer<List<DailyEnergy>>() {
                @Override

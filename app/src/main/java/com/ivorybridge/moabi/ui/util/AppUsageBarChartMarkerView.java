...
public class AppUsageBarChartMarkerView extends MarkerView {

    private static final String TAG = AppUsageBarChartMarkerView.class.getSimpleName();
    ...
    public AppUsageBarChartMarkerView(Context context, int layoutResource, Map<String, Long> AppUsageNameRankMap) {
        ...
        if (entryDatesList != null) {
            ...
            for (Map.Entry<String, Long> appSummary : AppUsageNameRankMap.entrySet()) {
                ...
                if (appSummary.getKey().length() > 5) {
                    appName = appSummary.getKey().substring(0, 5) + "...";
                } else {
            ...
            //Log.i(TAG, appSummary.getValue() + ", " + e.getX());
            //String time = convertToTimeString(e.getY());
    ...
    private String convertToTimeString(float timeInFloat) {
        ...
        /*
        if (hour > 100) {
            timeElapsed = String.format(Locale.US, "%03d:%02d", hour, minute);
        } else {
            timeElapsed = String.format(Locale.US, "%02d:%02d", hour, minute);
        }*/
        //Log.i(TAG, "" + timeInFloat);
        //String timeElapsed2 = String.format(Locale.US, "%1.1d hours", hour);
        return timeElapsed;
    }
}
